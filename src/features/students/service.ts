...
import type { Student, StudentCreateInput, StudentUpdateInput } from "./schema";
...
export async function updateStudent(
    ...
    if (patch.parentId !== undefined) {
        const parent = await parentsRepo.findParentById(patch.parentId);
        if (!parent) throw new AppError("PARENT_NOT_FOUND", 404);
    }
    const updated = await repo.updateStudent(id, patch);
    if (!updated) throw new AppError("STUDENT_NOT_FOUND", 404);
    return updated;
}
