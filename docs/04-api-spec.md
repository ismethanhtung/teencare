...
Cập nhật từng phần. Body chỉ chứa các trường cần đổi (`name`, `phone`, `email`). Truyền `email: null` để xoá email. Lỗi: `404 PARENT_NOT_FOUND`, `409 PARENT_PHONE_DUPLICATE` / `PARENT_EMAIL_DUPLICATE`.
...
```json
{
    "error": {
        "code": "CLASS_FULL",
        "message": "Class has reached max students",
        "details": { "classId": "..." }
    }
}
```
